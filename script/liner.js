...
module.exports.run = async function ({ api, event, args }) {
    try {
        ...
        if (!body || !body.toLowerCase().startsWith("liner")) {
            return;
        }
        ...
        if (!prompt) {
            return api.sendMessage(
                `Please provide a prompt to get a response from Liner.`,
                threadID,
                messageID
            );
        }
        ...
        const apiUrl = `https://api.joshweb.click/api/liner?q=${encodeURIComponent(prompt)}`;
        ...
        while (attempts < 3) {
            try {
                ...
                if (attempts >= 3) {
                    console.error(error);
                    return api.sendMessage(
                        `An error occurred while communicating with the Liner API. Please try again later.`,
                        threadID,
                        messageID
                    );
                }
